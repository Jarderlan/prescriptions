<?php 
	...
	#Filtro
	$filter['id_patient'] = $_POST['filter'];

	# Executando a exclusão no banco
	$manager->delete_common("tb_patient",$filter,null);

	# Redireciona para a lista de users
	header("location: $project_index/".$user->user_profile.".php?option=list_patients&success=patient_deleted&content=Usuários");
